...
class LinkedList:
    ...
    def remove_tail(self):
        ...
        current = self.head
        #loops through list as long as next item isn't tail
        #and sets current to the last value before tail
        while current.get_next() is not self.tail:
            current = current.get_next()

        value = self.tail.get_value()
        self.tail = current
        #had to add line below or line 35 would break test
...
class Stack:
    ...
    def __len__(self):
        ...
        return length
    #adds to last item in linked list
    def push(self, value):
        return self.storage.add_to_tail(value)
    #removes last item from linkedlist
    def pop(self):
